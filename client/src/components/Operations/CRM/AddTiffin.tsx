...
import { AppState, OverrideSchedule, SubscriptionSchedule } from '../../../type/Type';
...
const AddTiffin = (props) => {
  ...
  const subscriber:SubscriptionSchedule = props && props.location && props.location.subscriber;
  const apiError = useSelector((state:AppState) => state.apiError);
  
  
  const [overrideEndDate, setOverrideEndDate] = useState(dateFns.addDays(new Date(), 2));
  const [overrideStartDate, setOverrideStartDate] = useState(dateFns.addDays(new Date(), 1));
  ...
  const [newPlanOverride, setNewPlanOverride]  =  useState<OverrideSchedule | null>({ overrideStartDate,
                                                                                      overrideEndDate,
  ...
  const validateAndOverride = () => {
    ...
    if (isValidDate) {
      ...
      setIsBusy(true);
      const overridePlan: OverrideSchedule = Object.assign(
        {},
        newPlanOverride
      );
    ...
    <div  className="CRMDashboard-AddTiffin-container">
      ...
      <Alert color="info">
         <ul>
           ...
           <li>For Thali count change select count that would be actually delivered for selected duration.</li> 
         </ul>      
      </Alert>
      ...
      <FormControl>
        <InputLabel id="demo-simple-select-outlined-label">Number of Thalis Requested</InputLabel>
        <Select
